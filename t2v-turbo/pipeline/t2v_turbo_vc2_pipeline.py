...
class T2VTurboVC2Pipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                model_pred = self.unet(current_latent, ts, **context)
                # compute the previous noisy sample x_t -> x_t-1
                current_latent, denoised = self.scheduler.step(
                    model_pred, i, t, current_latent,
                    generator=generator, variance_noise=step_noise, return_dict=False
                )
    ...
    def forward_step(
            ...
            motion_embedding: Optional[torch.Tensor] = None,
            fps: int = 16,
            step_noise: Optional[torch.Tensor] = None,
            eta: float = 0.0,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Single diffusion step (no @torch.no_grad), for DNO.
        """
        bs = latents.shape[0]
        ts = torch.full((bs,), timestep, device=latents.device, dtype=torch.long)

        # build exactly the same context dict as __call__
        ctx: Dict[str, torch.Tensor] = {
            "context": prompt_embeds.to(self.dtype),
            "fps": fps,
            "timestep_cond": w_embedding.to(self.dtype),
        }
        if motion_embedding is not None:
            ctx["motion_cond"] = motion_embedding.to(self.dtype)

        # checkpointed UNet forward
        def _unet_fw(x, t, c):
            return self.unet(x, t, **c)

        model_pred = torch.utils.checkpoint.checkpoint(_unet_fw, latents, ts, ctx)
